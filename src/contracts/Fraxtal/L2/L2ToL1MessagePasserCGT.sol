...
import { Burn } from "@eth-optimism/contracts-bedrock/src/libraries/Burn.sol";
...
contract L2ToL1MessagePasserCGT is ISemver {
    ...
    // Frax: Raised to 200K because the ERC20 transfer-related gas on L1 is ~135K
    ...
    function initiateWithdrawal(address _target, uint256 _gasLimit, bytes memory _data) public payable {
        // // Frax: Raised to 200K because the ERC20 transfer-related gas logic on L1 is ~135K
        // if ((msg.value > 0) && (_gasLimit < RECEIVE_DEFAULT_GAS_LIMIT)) revert GasLimitTooLow();
    ...
    // /// @notice Thrown if the gas limit is set too low

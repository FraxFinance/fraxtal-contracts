...
// lib/optimism/packages/contracts-bedrock/lib/openzeppelin-contracts/contracts/utils/math/Math.sol
...
// lib/optimism/packages/contracts-bedrock/lib/openzeppelin-contracts/contracts/utils/math/SignedMath.sol
...
// lib/optimism/packages/contracts-bedrock/lib/solmate/src/utils/FixedPointMathLib.sol
...
// lib/optimism/packages/contracts-bedrock/src/libraries/Burn.sol
...
// lib/optimism/packages/contracts-bedrock/src/libraries/PortalErrors.sol
...
// lib/optimism/packages/contracts-bedrock/src/vendor/AddressAliasHelper.sol
...
// lib/optimism/packages/contracts-bedrock/src/libraries/Arithmetic.sol
...
// lib/optimism/packages/contracts-bedrock/src/L1/ResourceMetering.sol
...
contract OptimismPortalCGT is Initializable, ResourceMetering, ISemver {
    ...
    /// @param _assetRecipient Address of the recipient for the 1-time withdrawal of the frxETH and ETH
    function initialize(
        ...
        // Move out old assets. Only ever meant to be called once: during initialization after an L2 hardfork
        // ---------------------------
        // Move all frxETH to a designated recipient. We don't send to the L1StandardBridgeCGT directly because it has
        // a deposits mapping that tracks tokens, and dropping in tokens would mess up accounting. So this frxETH will have to be
        // re-bridged later, then we would need to burn some L2 tokens for proper accounting
        if (_frxEthErc20Address != address(0)) {
            SafeERC20.safeTransfer(
                IERC20(_frxEthErc20Address),
                _assetRecipient,
                IERC20(_frxEthErc20Address).balanceOf(address(this))
            );

            // Technically can be out of this scope, put prevents being called at the impl level, only the proxy level
            // Move all the ETH to a designated recipient
            (bool success, ) = _assetRecipient.call{ value: address(this).balance }("");
            if (!success) {
                revert TransferFailed();
            }
        }

        // Zero the _balance because the gas token is changing
        _balance = 0;

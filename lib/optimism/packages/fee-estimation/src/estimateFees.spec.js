...
import { baseFee, decimals, estimateFees, gasPrice, getL1Fee, getL1GasUsed, getL2Client, l1BaseFee, overhead, scalar, version, } from './estimateFees';
import { optimistABI, optimistAddress, l2StandardBridgeABI, l2StandardBridgeAddress, } from '@eth-optimism/contracts-ts';
import { parseEther, parseGwei } from 'viem';
vi.mock('viem', async () => {
    const _viem = (await vi.importActual('viem'));
    return {
        ..._viem,
...
test('estimateFees should return correct fees', async () => {
    ...
    expect(await estimateFees({ ...paramsWithRpcUrl, ...functionDataBurn })).toMatchInlineSnapshot('20573203833264n');
    expect(await estimateFees({ ...paramsWithViemClient, ...functionDataBurn })).toMatchInlineSnapshot('20573203833264n');
    ...
    expect(await estimateFees({ ...paramsWithRpcUrlWithdraw, ...functionDataWithdraw })).toMatchInlineSnapshot('62857090247510n');
    expect(await estimateFees({ ...paramsWithRpcUrlWithdraw, ...functionDataWithdraw })).toMatchInlineSnapshot('62857090247510n');
    expect(await estimateFees({ ...paramsWithRpcUrlWithdraw, ...functionDataWithdraw })).toMatchInlineSnapshot('62857090247510n');

 ...
 */
const directoryPath = process.argv[2] || path_1.default.join(__dirname, '..', 'forge-artifacts');
/**
 ...
 */
const parseVariableInfo = (variable) => {
    ...
    if (variable.type.startsWith('t_mapping')) {
        ...
        throw new Error(`${variable.label}: unsupported type ${variable.type}, add it to the script`);
    }
    return {
        name: variable.label,
...
const main = async () => {
    ...
    for (const filePath of paths) {
        ...
        const raw = fs_1.default.readFileSync(filePath, 'utf8').toString();
            ...
            for (const variable of storageLayout.storage) {
                ...
                if (variable.label.startsWith('spacer_')) {
                    ...
                    if (parseInt(slot, 10) !== variableInfo.slot) {
                        throw new Error(`${fqn} ${variable.label} is in slot ${variable.slot} but should be in ${slot}`);
                    }
                    // Check that the offset is correct.
                    if (parseInt(offset, 10) !== variableInfo.offset) {
                        throw new Error(`${fqn} ${variable.label} is at offset ${variable.offset} but should be at ${offset}`);
                    }
                    // Check that the length is correct.
                    if (parseInt(length, 10) !== variableInfo.length) {
                        throw new Error(`${fqn} ${variable.label} is ${variableInfo.length} bytes long but should be ${length}`);
                    }

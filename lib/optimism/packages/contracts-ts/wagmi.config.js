...
if (!glob.sync('node_modules/*').length) {
    throw new Error('No node_modules found. Please run `pnpm install` before running this script');
}
const deployments = {
    [1]: glob.sync(`node_modules/@eth-optimism/contracts-bedrock/deployments/${chains[1]}/*.json`),
    [10]: glob.sync(`node_modules/@eth-optimism/contracts-bedrock/deployments/${chains[10]}/*.json`),
    [5]: glob.sync(`node_modules/@eth-optimism/contracts-bedrock/deployments/${chains[5]}/*.json`),
    [420]: glob.sync(`node_modules/@eth-optimism/contracts-bedrock/deployments/${chains[420]}/*.json`),
};
...
const getWagmiContracts = (deploymentFiles, filterDuplicates = false) => deploymentFiles.map((artifactPath) => {
    ...
    const abi = filterDuplicates
        ? deployment.abi.filter((item) => {
            ...
            return !deployment.abi.some((otherItem) => otherItem.type === 'function' &&
    ...
    if (!contractConfig.name) {
        throw new Error('Unable to identify the name of the contract at ' + artifactPath);
    }
...
const getContractConfigs = (filterDuplicates = false) => {
    ...
    for (const contract of allContracts) {
        ...
        const alreadyProcessedContract = config.find((c) => c.name === contract.name);
        ...
        const predeploy = predeployContracts[contract.name];
        // If the contract has different abis on different networks we don't want to group them as a single abi
        const isContractUnique = !Object.values(implementations).some((implementation) => implementation && !isDeepStrictEqual(implementation.abi, contract.abi));
        if (!isContractUnique) {
            Object.entries(implementations)
                ...
                if (implementation) {
                    ...
                    const name = i === 0 ? contract.name : `${contract.name}_${chains[chain]}`;
                    const nextConfig = {
                        abi: implementation.abi,
                        name,
                        address: {
                            [Number.parseInt(chain)]: predeploy?.address ??
 ...
 */
const addressesByContractByNetworkPlugin = {
    name: 'addressesByContractByNetwork',
    run: async ({ contracts }) => {
        const addresses = Object.fromEntries(contracts.map((contract) => [contract.name, contract.address ?? {}]));
...
const abiPlugin = {
    ...
    run: async ({ contracts }) => {
        const abis = Object.fromEntries(contracts.map((contract) => [contract.name, contract.abi]));
        // write to json file so it's easy to audit in prs relative to the generated file diff

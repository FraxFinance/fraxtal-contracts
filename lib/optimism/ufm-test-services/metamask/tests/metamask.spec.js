...
import { incrementSelfSendTxGauge, setFeeEstimationGauge, } from './prometheusUtils';
const env = z
    .object({
...
const expectedSender = env.METAMASK_SECRET_WORDS_OR_PRIVATEKEY?.startsWith('0x')
    ? privateKeyToAccount(env.METAMASK_SECRET_WORDS_OR_PRIVATEKEY).address.toLowerCase()
    : mnemonicToAccount(env.METAMASK_SECRET_WORDS_OR_PRIVATEKEY).address.toLowerCase();
...
test('Send an EIP-1559 transaction and verify success', async () => {
    ...
    const notificationPage = await synpressPlaywright.switchToMetamaskNotification();
    console.log('Gathering transaction fee estimations...');
    const lowFeeEstimate = await getFeeEstimateInGwei(confirmPageElements.gasOptionLowButton, 'Low', notificationPage);
    const highFeeEstimate = await getFeeEstimateInGwei(confirmPageElements.gasOptionHighButton, 'Aggressive', notificationPage);
    // Medium needs to be last because that's the gas option we want to submit the tx with
    const mediumFeeEstimate = await getFeeEstimateInGwei(confirmPageElements.gasOptionMediumButton, 'Market', notificationPage);
    console.log('Sent transaction, waiting for confirmation...');
    await metamask.confirmTransactionAndWaitForMining();
    const txHash = await txHashPromise;
    const transactionReceiptPromise = new Promise((resolve) => {
        sharedPage.on('load', async () => {
            const responseText = await sharedPage.locator('body > main').innerText();
            const transactionReceipt = JSON.parse(responseText.replace('Response: ', ''));
    ...
    await sharedPage.goto(`${env.METAMASK_DAPP_URL}/request.html?method=eth_getTransactionReceipt&params=["${txHash}"]`);
...
const getFeeEstimateInGwei = async (gasOptionButton, waitForText, notificationPage) => {
    await synpressPlaywright.waitAndClick(confirmPageElements.editGasFeeButton, notificationPage);
    await synpressPlaywright.waitAndClick(gasOptionButton, notificationPage);
    await synpressPlaywright.waitForText(`${confirmPageElements.editGasFeeButton} .edit-gas-fee-button__label`, waitForText, notificationPage);
    const regexParseEtherValue = /(\d+\.\d+)\s?OPS/;
    const feeValue = (await synpressPlaywright.waitAndGetValue(confirmPageElements.totalLabel, notificationPage)).match(regexParseEtherValue)[1];
    return parseInt(parseGwei(feeValue).toString());
};
const getActualTransactionFee = (transactionReceipt) => {

...
const env = z
    ...
    .refine((data) => {
    if ((data.METRICS_READ_USERNAME && !data.METRICS_READ_PASSWORD) ||
        (data.METRICS_READ_PASSWORD && !data.METRICS_READ_USERNAME)) {
        return false;
    }
    if ((data.METRICS_WRITE_USERNAME && !data.METRICS_WRITE_PASSWORD) ||
        (data.METRICS_WRITE_PASSWORD && !data.METRICS_WRITE_USERNAME)) {
        return false;
    }
    return true;
}, {
    message: 'Both username and password must be provided together for read or write metrics',
})
    .refine((data) => {
    if (data.METRICS_WRITE_TOOL === 'grafana' &&
        data.METRICS_WRITE_SOURCE === undefined)
        return false;
    return true;
}, {
    message: 'Writing to Grafana requires a source, please specify one using METRICS_WRITE_SOURCE env',
})
    .parse(process.env);
...
const feeEstimateMediumMetricName = 'metamask_self_send_fee_estimation_medium_metric';
const feeEstimateHighMetricName = 'metamask_self_send_fee_estimation_high_metric';
const feeEstimateActualMetricName = 'metamask_self_send_fee_estimation_actual_metric';
...
const queryMetricsReadUrl = async (query = selfSendTransactionMetricName) => {
    const metricsReadRequest = `${env.METRICS_READ_URL}?query=${query}`;
    const response = await fetch(metricsReadRequest, {
        headers: env.METRICS_READ_USERNAME === undefined
            ? undefined
            : {
...
export const getSelfSendGaugeValue = async () => {
    ...
    try {
        ...
        if (error.message === "Cannot read properties of undefined (reading 'value')") {
            console.warn(`No data found for metric ${selfSendTransactionMetricName} in ${env.METRICS_READ_URL}`);
            return undefined;
        }
        throw error;
    }
};
const pushMetricsGrafana = (metricName, valueToSetTo) => pushMetricsWriteUrl(`${metricName},source=${env.METRICS_WRITE_SOURCE} metric=${valueToSetTo}`);
const pushMetricsPrometheusPushgateway = (registry) => {
...
const pushMetricsWriteUrl = async (requestBody) => {
    const response = await fetch(env.METRICS_WRITE_URL, {
        method: 'POST',
        headers: env.METRICS_WRITE_USERNAME === undefined
            ? undefined
            : {
...
export const setSelfSendTxGauge = async (valueToSetTo) => {
...
export const incrementSelfSendTxGauge = async (isSuccess) => {
    ...
    console.log(`Current value of ${selfSendTransactionMetricName} is ${currentMetricValue}, incrementing to ${newMetricValue}...`);
    await setSelfSendTxGauge(newMetricValue);
};
export const setFeeEstimationGauge = async (txSpeed, fee) => {
    let metricNameGrafana;
